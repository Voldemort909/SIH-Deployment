...
2. **Configure Environment (`.env`)**:
   ...
   *(On Linux/macOS: `cp .env.example .env`)*
   ...
   Start a local MySQL server or Cloud SQL instance, then configure `.env`:
   - Set a secure random `SECRET_KEY`
   - Configure `DEMO_USER_PASSWORD` (used when running the demo seed script)
   - Set `DATABASE_URL` (e.g. `mysql+pymysql://<user>:<password>@<host>:<port>/<dbname>`) for your Cloud SQL or MySQL database
...
2. In the web service environment, set `DATABASE_URL` to your Cloud SQL or managed MySQL connection string (`mysql+pymysql://<user>:<password>@<host>:<port>/<dbname>`).
3. `SECRET_KEY` is generated by the Blueprint. Set `CORS_ORIGINS` to `*` for the bundled frontend, or provide comma-separated origins if hosting it separately. `FLASK_ENV=production` is set by the Blueprint.
...
| `DATABASE_URL` | Cloud SQL / MySQL connection string (`mysql+pymysql://<user>:<password>@<host>:<port>/<dbname>`) |
